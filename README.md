...
- 核心状态 `Σ=(P,x,y,h,L,R,C)`：当前地图、坐标、水平朝向、两个整数资源 `L,R∈ℤ`、调用栈；
...
正式规范见 [`doc.md`](doc.md)——包含运动函数 `D(c,L,R,h)` 与执行函数 `Exec` 的完整定义。
...
- 从内置示例下拉切换（min / gate / recursion / fib 斐波那契 / **nth-prime 第 n 个质数**…）；
...
`nth-prime.xfeng` 是运行时算法，不是质数表。`F` 用普通递归深度保存 `n`；每次返回调用 `X`
寻找下一个候选数。`D` 用可逆递归减法判断整除，返回前恢复候选数与除数（`B` 已并入 `D`，
`a` 已并入 `A`）；`K` 只有在最小除数等于候选数本身时才判为质数。因此对每个 `n>=1`，程序
都会在有限步后得到 `p_n`。源码不含哥德尔数、素因子指数打包或预计算质数，非注释部分
41 行，最长一行 14 字符；9 个函数（`main`、`F`、`X`、`C`、`K`、`D`、`G`、`H`、`A`）。

这是朴素试除，标准逐步解释器适合观察较小输入。仓库另带一个逐 tick、语义等价的 Numba
验证器用于大规模核验；它不是替换算法，也不直接调用宿主语言的求质数函数：

```bash
python -m pip install numpy numba
python tools/verify_nth_prime.py --count 500
```

本机实测 `(500,0) -> 3571`，共执行 57,651,270,287 个 XFeng tick，约 188 秒；同一轨迹中的
500 个连续质数检查点全部与独立的埃拉托斯特尼筛法一致。验证器为了速度使用固定宽度整数和
有限预分配栈；这些是验证工具的实现限制，不是 XFeng 源程序或语言语义的上界。
...
1. **函数名区分大小写**（doc §5）：`@A` 与 `@a` 是两个不同函数，可共存；同名重复声明报错。
